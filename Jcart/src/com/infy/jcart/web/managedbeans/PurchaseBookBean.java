...
@ManagedBean
@SessionScoped
public class PurchaseBookBean {
	private String subCategoryName;
	private List<Books> productList;
	...
	public void setProductList(List<Books> productList) {
	...
	/****
	 * @description This method retives the selected subcategory
	 * 
	 * @exception Exception
	 */
	public PurchaseBookBean() {
		try {
			...
			this.productList = new ArrayList<Books>();
			this.errorMessage = "";
			this.errorMessage = "";
			this.productList.clear();
		} catch (Exception exception) {
			JCartLogger.logError(this.getClass().getName(), "PurchaseBookBean", exception
					.toString());
	...
	/***
	 * @description This method is used in displaying book details under the selected sub category
	 * @return String
	 * @exception Exception
	 * 
	 */
	public String purchaseBook() {
		...
		try {
			...
			request = (HttpServletRequest) etx.getRequest();
			this.subCategoryName = request.getParameter("subCategory");
			this.productList = Factory.createPurchaseService().getAllBookProducts(subCategoryName);
					
		}  catch (Exception exception) {
			JCartLogger.logError(this.getClass().getName(), "purchaseBook", exception
					.toString());
			if(exception.getMessage().contains("DAO.TECHNICAL_ERROR")){
				ErrorRedirect.reDirect();
